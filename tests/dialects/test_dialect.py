...
class Validator(unittest.TestCase):
    ...
    def validate_all(self, sql, read=None, write=None, pretty=False, identify=False):
        ...
        if read is None:
            ...
            raise TypeError(
                f"read must be a mapping of dialect->sql or None, got {type(read).__name__}"
            )

        for read_dialect, read_sql in read_map.items():
        ...
        if write is None:
            write_map = {}
        elif isinstance(write, Mapping):
            write_map = write
        else:
            raise TypeError(
                f"write must be a mapping of dialect->sql or None, got {type(write).__name__}"
            )

        for write_dialect, write_sql in write_map.items():
